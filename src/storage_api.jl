...
"""
    evict_derived!(predicate, runtime::Runtime) -> Int

Delete every memoized derived value whose key matches `predicate`, and return the number
of evicted entries. The predicate is called with a `DerivedKey` (the derived function is
its type parameter, the call's arguments are in `.args`) and must return a `Bool`.

Eviction is purely a cache operation: an evicted value is recomputed the next time its
derived function is called with that key. Like input mutation, this may not be called
while any derived function is running.
"""
function evict_derived! end
